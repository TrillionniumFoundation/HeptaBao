...
/// Verify an imported Go bcrypt hash with an already decoded salt.
///
/// Go's fixed 22-byte salt text may contain CR/LF ignored by its base64
/// decoder, producing a shorter salt. This compatibility entry accepts only
/// nonempty salts of at most 16 bytes, cost 5..=12, and exactly 31 bytes of
/// encoded hash. Passwords use the same 72-byte input rule as `verify`.
/// The caller owns header/base64 admission; this function never generates
/// nonstandard hashes or changes the existing fixed-salt APIs.
#[cfg(any(feature = "alloc", feature = "std"))]
pub fn verify_with_decoded_salt(
    password: &[u8],
    cost: u32,
    salt: &[u8],
    expected_hash: &[u8],
    ...
    if salt.is_empty() || salt.len() > 16 || expected_hash.len() != 31 {
        return Err(BcryptError::InvalidHash("invalid decoded-salt input"));
    }
    let mut pass = [0u8; 72];
    let copy_len = password.len().min(72);
    pass[..copy_len].copy_from_slice(&password[..copy_len]);
    let used = (copy_len + 1).min(72);
    let mut output = bcrypt::bcrypt_with_salt(cost, salt, &pass[..used]);
    #[cfg(feature = "zeroize")]
    pass.zeroize();
    let mut encoded = [0u8; 31];
    let written = BASE_64.encode_slice(&output[..23], &mut encoded);
    #[cfg(feature = "zeroize")]
    output.zeroize();
    let result = match written {
        Ok(31) => Ok(bool::from(encoded.as_slice().ct_eq(expected_hash))),
        _ => Err(BcryptError::InvalidHash("invalid encoded hash length")),
    };
    #[cfg(feature = "zeroize")]
    encoded.zeroize();
    result
