...
pub fn bcrypt(cost: u32, salt: [u8; 16], password: &[u8]) -> [u8; 24] {
    bcrypt_with_salt(cost, &salt, password)
}

// Same inner algorithm for the existing fixed-salt API and the bounded Go
// verification adapter. No second key schedule or round implementation.
pub(crate) fn bcrypt_with_salt(cost: u32, salt: &[u8], password: &[u8]) -> [u8; 24] {
    assert!(!salt.is_empty() && salt.len() <= 16);
