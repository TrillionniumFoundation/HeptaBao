...
| `qa/openbao-acceptance/online_auth_ha.py` | Three real same-host service processes, official issuer, controlled reviewer, leader death, no quorum and concurrent callback consumption. The first post-session failover observes committed quorum and then a stable application-ready leader through read-only probes; it never retries an auth URL, callback or provider effect. Not physical multi-host or complete distributed fault coverage. |
