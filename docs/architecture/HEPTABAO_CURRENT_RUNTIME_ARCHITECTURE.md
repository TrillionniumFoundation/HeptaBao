...
| Structural entities, aliases, groups and merge lineage | `NamespaceState.identity` inside EngineState and the same Service transaction; the Service additionally binds login identities and projects live internal-group policies; complete MFA/OIDC remain separate | `identity/*`; `engines/identity.rs` tests |
