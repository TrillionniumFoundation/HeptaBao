...
| 49 | Reserved by the parallel workflow-state candidate; not admitted by this Kerberos-only integration branch because that reader is not present. Integration must preserve both independent feature checks before admitting workflow state. |
