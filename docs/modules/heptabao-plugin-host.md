...
These process and durable broker implementations are now consumed by the server's explicitly configured dynamic-secret path. The runnable server admits one checksum-bound Secrets/Database plugin plus sandbox wrapper and exposes provider-native HeptaBao issue/renew/revoke and readback routes. This does **not** create a general plugin catalog or OpenBao plugin gRPC ABI, does not qualify a real database/cloud connector or operating-system sandbox, and is deliberately single-active: server startup rejects dynamic-secret configuration together with HA.
