...
Paths passed to Service omit `/v1/` and the root namespace is empty; the HTTPS parser owns wire canonicalization and bounds. `http::Config` owns absolute storage/audit/TLS paths, listen address, connection/deadline/rate-limit settings, audit retention configuration and optional `dynamic_secrets` provider admission. The dynamic configuration pins plugin and sandbox commands by SHA-256 and uses a separate encrypted state directory; it is rejected when HA is configured. Unknown JSON configuration fields are rejected. `http::serve` owns configuration and starts the listener; `serve_with_ha` shares the HA process. The private `auth` and `service` modules are not exported, even where historical tables below show `pub` declarations.
