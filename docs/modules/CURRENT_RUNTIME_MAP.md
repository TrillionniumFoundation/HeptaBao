...
| `heptabao-domain` | yes | Bounded plugin identifiers, paths and secret buffers for the server read-only plugin runtime | `crates/heptabao-domain/src/lib.rs::identifier_and_path_validation_are_fail_closed` |
...
| `heptabao-identity` | no | Standalone Entity/alias/group model; current identity/* routes use server/engines/identity.rs | `crates/heptabao-identity/src/lib.rs::aliases_groups_and_direct_policies_expand_deterministically` |
...
| `heptabao-kms-contracts` | yes | Key/version/capability contract used by the server checksum-pinned KMS plugin routes; auto-unseal remains uncomposed | `crates/heptabao-kms-contracts/src/lib.rs::key_lifecycle_is_fail_closed_and_monotonic` |
...
| `heptabao-plugin-contracts` | yes | Descriptor/generation/capability admission for deployment-enrolled server plugin runtimes | `crates/heptabao-plugin-contracts/src/lib.rs::lifecycle_is_monotonic_after_revocation` |
| `heptabao-plugin-host` | yes | Sealed executable host for bounded server-integrated read-only secret plugins; write/lease effects remain outside the admitted path | `crates/heptabao-plugin-host/src/durable.rs::issued_secret_is_released_only_after_durable_metadata_and_survives_reopen` |
...
| `heptabao-server` | yes | Current executable: TLS/Service auth, seal/rekey, KV, Transit, TOTP, token-private Cubbyhole, bounded live Identity/JWT, SSH OTP, internal PKI, maintenance and optional network HA including explicit leadership transfer; /v1/* | `crates/heptabao-server/src/service_tests.rs::result_audit_failure_withholds_plaintext_and_preserves_consumed_token_after_reopen` |
