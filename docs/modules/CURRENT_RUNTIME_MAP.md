...
This map describes the current source as derived from the exact Git tree and live source inventory; the policy JSON is not a second source authority, and this map is not a test-pass receipt. The table contains every current workspace package. Runtime membership is derived from the normal path-dependency closure of `heptabao-server`; packages outside that runtime closure remain independently useful contracts, models, prototypes or tooling, and their tests do not establish that the corresponding feature is integrated into the running service. Dev dependencies are not runtime edges. Dependencies external to the workspace are omitted here and remain pinned by Cargo.lock.
