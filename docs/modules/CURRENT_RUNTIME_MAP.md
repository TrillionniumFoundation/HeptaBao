...
| `heptabao-domain` | yes | Bounded IDs, canonical paths and secret values used by the integrated dynamic-secret runtime; core KV/auth still use server-owned models | `crates/heptabao-domain/src/lib.rs::identifier_and_path_validation_are_fail_closed` |
...
| `heptabao-plugin-contracts` | yes | Descriptor/registry contracts used to admit the configured dynamic-secret provider; no generic OpenBao plugin catalog route | `crates/heptabao-plugin-contracts/src/lib.rs::lifecycle_is_monotonic_after_revocation` |
| `heptabao-plugin-host` | yes | Configured dynamic-secret provider host plus durable issue/renew/revoke intent and lease projection; reached by sys/dynamic-secrets/* and sys/leases/* | `crates/heptabao-plugin-host/src/durable.rs::issued_secret_is_released_only_after_durable_metadata_and_survives_reopen` |
...
| `heptabao-server` | yes | Current executable: TLS/Service auth, seal/rekey, KV, Transit, TOTP, configured dynamic SecretLease lifecycle, maintenance and optional network HA; /v1/* | `crates/heptabao-server/src/service_tests.rs::result_audit_failure_withholds_plaintext_and_preserves_consumed_token_after_reopen` |
