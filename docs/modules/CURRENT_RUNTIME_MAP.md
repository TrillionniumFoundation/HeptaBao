...
| `heptabao-server` | yes | Current executable: TLS/Service auth, seal/rekey, KV, Transit, TOTP, token-private Cubbyhole, structural Identity, maintenance and optional network HA; /v1/* | `crates/heptabao-server/src/service_tests.rs::result_audit_failure_withholds_plaintext_and_preserves_consumed_token_after_reopen` |
