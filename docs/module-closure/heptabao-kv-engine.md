# heptabao-kv-engine module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-kv-engine`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** versioned KV secrets engine
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-kv-engine`; Rust files: `crates/heptabao-kv-engine/src/lib.rs`.
...
- **Public design surface:** struct `KvMetadata`; struct `KvRead`; struct `KvStore`; fn `new`; fn `current_version`; fn `write`; fn `read`; fn `delete_latest`; fn `undelete`; fn `destroy`; fn `list`; enum `KvError`
...
The source-defined failure vocabulary is: `KvError::InvalidMaxVersions`; `KvError::MissingKey`; `KvError::MissingVersion`; `KvError::CasMismatch`; `KvError::VersionOverflow`; `KvError::Deleted`; `KvError::Destroyed`; `KvError::NotDeleted`
...
- **Source/manifest evidence:** source tree SHA-256 `b5550407dfe6689388a8bdba673fc82b4a44b5608cafec7e4ae265c246d094c9`; manifest SHA-256 `1548333d3e46ddb77572406809229ef0e8ade28f39e10a84d2d76f0c096192d0`.
