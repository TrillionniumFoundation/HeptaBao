# heptabao-single-node-store module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-single-node-store`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** local durable store
- **Repository state:** `INHERITED_IMPLEMENTED`.
- **Source root:** `crates/heptabao-single-node-store`; Rust files: `crates/heptabao-single-node-store/src/lib.rs`.
- **Internal dependencies:** `heptabao-filesystem-guard`, `heptabao-storage-api`.
...
- **Public design surface:** struct `FileGenerationStore`; fn `create_new`; fn `reopen_existing`; fn `adopt_legacy`; fn `root`; fn `root_identity`; enum `FileStoreError`
...
The source-defined failure vocabulary is: `DecodeError::Truncated`; `DecodeError::Trailing`; `DecodeError::InvalidMagic`; `DecodeError::InvalidLength`; `DecodeError::InvalidUtf8`; `DecodeError::InvalidGeneration`; `DecodeError::InvalidDigest`; `TestIntegrityError::Contract`
...
- **Source/manifest evidence:** portable repository-relative source SHA-256 `78226ca5bf76870084ed17288f8dd59da72981cf6c6eb9cd6f6d3573397226db`; manifest SHA-256 `5bdf319b0ad7bb439ac31c2f5a24364a1ce8f972aa540141c0008d565e1751b4`.
- **Named executable anchor:** `create_commit_load_and_reopen_round_trip` in `crates/heptabao-single-node-store/src/lib.rs`.
- **Required command:** `cargo +1.98.0 test --locked -p heptabao-single-node-store` (must be executed against this exact source tree; historical CI output is not current evidence).
