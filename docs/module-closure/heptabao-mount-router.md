# heptabao-mount-router module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-mount-router`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** namespace scoped mount routing
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-mount-router`; Rust files: `crates/heptabao-mount-router/src/lib.rs`.
...
- **Public design surface:** enum `Backend`; struct `Mount`; fn `id`; fn `namespace_id`; fn `path`; fn `backend`; struct `Route`; struct `MountRouter`; fn `mount`; fn `set_enabled`; fn `unmount`; fn `route`; enum `MountError`
...
The source-defined failure vocabulary is: `MountError::DuplicateMount`; `MountError::DuplicatePath`; `MountError::MissingMount`; `MountError::NoRoute`; `MountError::NoStateChange`
...
- **Source/manifest evidence:** source tree SHA-256 `e1f3c0995f91c1444a7eb48b1d40cf374edfc7e76272e4a1aee3069e11d25d7b`; manifest SHA-256 `9a0fa2697289993cf1ce471fc686ee3aab42874a777a18424cef2bac58cde78e`.
