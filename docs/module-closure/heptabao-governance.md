# heptabao-governance module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-governance`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** qualification and authority contracts
- **Repository state:** `INHERITED_IMPLEMENTED`.
- **Source root:** `crates/heptabao-governance`; Rust files: `crates/heptabao-governance/src/lib.rs`.
...
- **Public design surface:** enum `AuthorityEffect`; struct `QualificationFacts`; struct `TestSummary`; struct `FindingSummary`; enum `QualificationError`; const `fn`; struct `PlanningAuthority`; const `H00_AUTHORITY`
...
The source-defined failure vocabulary is: `QualificationError::DirtySourceTree`; `QualificationError::InvalidDependencyReceipt`; `QualificationError::RequiredLaneNotPassed`; `QualificationError::FailedTest`; `QualificationError::UnknownTestOutcome`; `QualificationError::CriticalFinding`; `QualificationError::HighFinding`; `QualificationError::UnclassifiedFinding`; `QualificationError::ExitGateNotPassed`; `QualificationError::InvalidApprovalSet`; `QualificationError::InvalidSignature`; `QualificationError::ExpiredSupersededOrRevoked`
...
- **Source/manifest evidence:** portable repository-relative source SHA-256 `972f4becdb2c03e3be8e3d93e89211d4898ccb3f294df675b09edbefa35813e7`; manifest SHA-256 `2fae46fc7c92cf7fcaa6374a58b2be49de8e2c9ae03cdedb0bf5b9dc1194ef31`.
- **Named executable anchor:** `qualification_never_grants_authority` in `crates/heptabao-governance/src/lib.rs`.
- **Required command:** `cargo +1.98.0 test --locked -p heptabao-governance` (must be executed against this exact source tree; historical CI output is not current evidence).
