# heptabao-key-lifecycle module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-key-lifecycle`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** key lifecycle contracts
- **Repository state:** `INHERITED_IMPLEMENTED`.
- **Source root:** `crates/heptabao-key-lifecycle`; Rust files: `crates/heptabao-key-lifecycle/src/lib.rs`.
- **Internal dependencies:** `heptabao-barrier-api`, `heptabao-journal-api`.
...
- **Public design surface:** const `MAX_REASON_CODE_BYTES`; struct `ReasonCode`; fn `new`; fn `as_str`; enum `KeyStatus`; enum `KeyUseDirective`; enum `KeyRingEventKind`; struct `KeyRingEvent`; const `fn`; const `fn`; const `fn`; fn `reason`; fn `encode`; fn `decode`; struct `KeyRingState`; const `fn`; fn `status`; fn `directive`; fn `known_epoch_count`; enum `KeyLedgerWriteState`; struct `KeyRingLedger`; fn `open`; const `fn`; const `fn`; const `fn`; const `fn`; fn `bootstrap`; fn `stage`; fn `rotate`; fn `retire`
...
The source-defined failure vocabulary is: `KeyLifecycleContractError::InvalidReasonCode`; `KeyLifecycleContractError::LengthOverflow`; `KeyLifecycleContractError::UnsupportedEventVersion`; `KeyLifecycleContractError::MalformedEvent`; `KeyLifecycleContractError::TrailingEventBytes`; `KeyLifecycleContractError::InvalidEventShape`; `KeyLifecycleContractError::AlreadyBootstrapped`; `KeyLifecycleContractError::NotBootstrapped`; `KeyLifecycleContractError::InvalidEpochOrder`; `KeyLifecycleContractError::InvalidTransition`; `KeyLifecycleContractError::ActiveRevocationForbidden`; `KeyLifecycleContractError::InvariantViolation`; `MemoryError::Contract`; `MemoryError::StaleTail`
...
- **Source/manifest evidence:** portable repository-relative source SHA-256 `00c40d9f39d060314090ad154d1deddb8afdb3f2076e2134044004f0ff8853bc`; manifest SHA-256 `0370dd97e37a4cb1ffb93c116bef98d4d1cc99f8efb0d382e2d79b814b28db04`.
- **Named executable anchor:** `bootstrap_stage_rotate_retire_and_revoke_replay` in `crates/heptabao-key-lifecycle/src/lib.rs`.
- **Required command:** `cargo +1.98.0 test --locked -p heptabao-key-lifecycle` (must be executed against this exact source tree; historical CI output is not current evidence).
