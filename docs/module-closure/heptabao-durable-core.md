# heptabao-durable-core module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-durable-core`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** durable commit contracts
- **Repository state:** `INHERITED_IMPLEMENTED`.
- **Source root:** `crates/heptabao-durable-core`; Rust files: `crates/heptabao-durable-core/src/lib.rs`.
- **Internal dependencies:** `heptabao-barrier-api`, `heptabao-storage-api`.
...
- **Public design surface:** struct `DurableStateEngine`; const `fn`; const `fn`; const `fn`; struct `PreparedDurableMutation`; const `fn`; fn `prepare_persist`; fn `commit_prepared`; fn `recover_commit`; fn `persist`; fn `load_current`; struct `LoadedSecretState`; enum `DurableCoreError`
...
The source-defined failure vocabulary is: `MemoryStoreError::Contract`; `MemoryStoreError::Conflict`; `MockBarrierError::Contract`; `MockBarrierError::AuthenticationFailed`
...
- **Source/manifest evidence:** portable repository-relative source SHA-256 `8733a9d925f8f16120525dd0fa8ef648273b74dadd453533c9811e4d306e349f`; manifest SHA-256 `6b02e9b04ac76daf4913919c2ebb1fd454effe9dd2a1476494d1ce2d9afcdcdb`.
- **Named executable anchor:** `prepared_mutation_binds_target_before_authoritative_commit` in `crates/heptabao-durable-core/src/lib.rs`.
- **Required command:** `cargo +1.98.0 test --locked -p heptabao-durable-core` (must be executed against this exact source tree; historical CI output is not current evidence).
