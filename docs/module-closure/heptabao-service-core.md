# heptabao-service-core module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-service-core`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** mandatory end to end service composition
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-service-core`; Rust files: `crates/heptabao-service-core/src/lib.rs`.
- **Internal dependencies:** `heptabao-domain`, `heptabao-identity`, `heptabao-kv-engine`, `heptabao-mount-router`, `heptabao-namespace`, `heptabao-operator-api`, `heptabao-policy`, `heptabao-telemetry`, `heptabao-token`.
...
- **Public design surface:** enum `ServiceOperation`; struct `ServiceRequest`; const `DEFAULT_REQUEST_REGISTRY_CAPACITY`; struct `RequestRegistryCounts`; const `fn`; enum `ServiceOutput`; enum `ServiceResponse`; struct `PostCommitError`; trait `PostCommitHook`; struct `NoopPostCommitHook`; struct `FailOncePostCommitHook`; struct `ServiceCore`; fn `new`; fn `new_with_request_capacity`; fn `identities_mut`; fn `policies_mut`; fn `tokens_mut`; fn `namespaces_mut`; fn `mounts_mut`; fn `kv`; fn `telemetry`; fn `reconciliation`; fn `request_registry_counts`; fn `resolve_unknown`; fn `handle`; enum `ServiceError`
...
The source-defined failure vocabulary is: `ServiceError::DuplicateRequest`; `ServiceError::RequestBindingMismatch`; `ServiceError::RequestRegistrySaturated`; `ServiceError::RequestRegistryInvariant`; `ServiceError::InvalidRequestRegistryCapacity`; `ServiceError::MissingRecoveryReference`; `ServiceError::RecoveryReferenceOverflow`; `ServiceError::Unauthorized`; `ServiceError::UnsupportedBackend`; `ServiceError::Domain`; `ServiceError::Identity`; `ServiceError::Token`; `ServiceError::Namespace`; `ServiceError::Mount`; `ServiceError::Kv`; `ServiceError::Telemetry`; `ServiceError::Operator`
...
- **Source/manifest evidence:** portable repository-relative source SHA-256 `b715bbcd79f3bcfdbef1c84d402f492fc501aad8f50096a44ce82f8f8953cf67`; manifest SHA-256 `087b15eaee03ade930f3144d443ab3c23bd8ab1f35a78d1370ad2647a80df100`.
- **Named executable anchor:** `accepted_request_runs_identity_policy_namespace_mount_and_engine` in `crates/heptabao-service-core/src/lib.rs`.
- **Required command:** `cargo +1.98.0 test --locked -p heptabao-service-core` (must be executed against this exact source tree; historical CI output is not current evidence).
