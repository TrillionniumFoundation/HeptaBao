# heptabao-storage-api module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-storage-api`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** storage contracts
- **Repository state:** `INHERITED_IMPLEMENTED`.
- **Source root:** `crates/heptabao-storage-api`; Rust files: `crates/heptabao-storage-api/src/lib.rs`.
...
- **Public design surface:** const `MAX_OPAQUE_STATE_BYTES`; const `MAX_STORE_DOMAIN_BYTES`; const `MAX_INTEGRITY_ALGORITHM_ID_BYTES`; struct `StoreDomain`; fn `new`; fn `as_str`; struct `IntegrityAlgorithmId`; fn `new`; fn `as_str`; struct `Generation`; const `INITIAL`; const `fn`; const `fn`; const `fn`; const `fn`; struct `StateDigest`; fn `new`; const `fn`; struct `OpaqueState`; fn `new`; fn `as_bytes`; fn `len`; fn `is_empty`; fn `into_bytes`; enum `StoreOpenMode`; struct `GenerationSnapshot`; struct `CommitReceipt`; struct `CommitIntent`; fn `new`; const `fn`
...
The source-defined failure vocabulary is: `StorageContractError::InvalidStoreDomain`; `StorageContractError::InvalidIntegrityAlgorithmId`; `StorageContractError::ZeroGeneration`; `StorageContractError::GenerationOverflow`; `StorageContractError::ZeroStateDigest`; `StorageContractError::InvalidOpaqueState`; `StorageContractError::InvalidCommitIntent`
