# heptabao-client-contracts module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-client-contracts`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** client retry classification
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-client-contracts`; Rust files: `crates/heptabao-client-contracts/src/lib.rs`.
...
- **Public design surface:** enum `OperationClass`; enum `FailureClass`; enum `RetryDecision`; struct `ClientAttempt`; fn `decision`; fn `next_with_new_id`
...
- **Source/manifest evidence:** source tree SHA-256 `a5f72a8befd8a60b66993f684ef8c9fab5da5f0d42bed0ebd204b92755334af1`; manifest SHA-256 `3cb2674bb8da7d545ab93e3009c3e91c330a25e35cb88a4a1140fca6a0b81bad`.
