# heptabao-kms-contracts module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-kms-contracts`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** provider-neutral KMS lifecycle and outcomes
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-kms-contracts`; Rust files: `crates/heptabao-kms-contracts/src/lib.rs`.
...
- **Public design surface:** const `MAX_WRAPPED_VALUE_BYTES`; enum `KmsCapability`; enum `KeyState`; struct `KeyVersion`; fn `new`; const `fn`; struct `KeyRegistration`; struct `KeyView`; struct `KeyCatalog`; fn `register`; fn `view`; fn `require_operation`; fn `disable`; fn `enable`; fn `schedule_destruction`; fn `destroy`; struct `WrappingContext`; fn `new`; struct `WrappedValue`; fn `new`; fn `expose`; fn `len`; fn `is_empty`; struct `WrapCommand`; struct `UnwrapCommand`; enum `RetryDisposition`; enum `KmsOutcome`; fn `retry_disposition`; trait `KmsProvider`; enum `KmsError`
...
The source-defined failure vocabulary is: `KmsError::InvalidKeyVersion`; `KmsError::InvalidAssociatedDataDigest`; `KmsError::InvalidWrappedValue`; `KmsError::MissingCapability`; `KmsError::DuplicateKey`; `KmsError::MissingKey`; `KmsError::KeyVersionMismatch`; `KmsError::KeyDisabled`; `KmsError::KeyPendingDestruction`; `KmsError::KeyDestroyed`; `KmsError::CapabilityDenied`; `KmsError::InvalidKeyTransition`; `KmsError::InvalidDestructionDeadline`; `KmsError::DestructionNotDue`; `KmsError::ProviderUnavailableBeforeEntry`; `KmsError::ProviderRejected`; `KmsError::GenerationOverflow`
...
- **Source/manifest evidence:** source tree SHA-256 `9d2204004e611261441c2475c0f9cb9bc4c6c6357345b79126c00073f5fa3a5e`; manifest SHA-256 `1f1ab2ce86c8a398a7fd2c7fbca106b861ad8caee0271b43c64b4dd420b2b6c9`.
