# heptabao-operator-api module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-operator-api`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** operator outcome classification
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-operator-api`; Rust files: `crates/heptabao-operator-api/src/lib.rs`.
...
- **Public design surface:** enum `EffectPhase`; enum `CommitState`; enum `OperatorAction`; enum `Resolution`; struct `OutcomeRecord`; fn `before_entry`; fn `committed`; fn `unknown_after_entry`; fn `action`; struct `ReconciliationStore`; fn `record`; fn `get`; fn `resolve`; enum `OperatorError`
...
The source-defined failure vocabulary is: `OperatorError::DuplicateRequest`; `OperatorError::MissingRecord`; `OperatorError::AlreadyResolved`
...
- **Source/manifest evidence:** portable repository-relative source SHA-256 `5f9612d1ce396f5e2a14ef5141477c66e714b5b48f6798335a7b50624d14350f`; manifest SHA-256 `98574a3c782be1c8e837b7997d0e1f1356b661a4a041ac34bd1f737581366b15`.
- **Named executable anchor:** `unknown_after_entry_forbids_retry_until_readback` in `crates/heptabao-operator-api/src/lib.rs`.
- **Required command:** `cargo +1.98.0 test --locked -p heptabao-operator-api` (must be executed against this exact source tree; historical CI output is not current evidence).
