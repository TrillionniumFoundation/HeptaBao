# heptabao-retention module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-retention`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** retention and backup lifecycle
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-retention`; Rust files: `crates/heptabao-retention/src/lib.rs`.
...
- **Public design surface:** struct `RetentionPolicy`; fn `validate`; fn `versions_to_prune`; enum `BackupState`; struct `BackupReceipt`; struct `BackupCoordinator`; fn `state`; fn `generation`; fn `begin`; fn `seal`; fn `verify`; fn `fail`; enum `RetentionError`
...
The source-defined failure vocabulary is: `RetentionError::InvalidPolicy`; `RetentionError::InvalidTransition`; `RetentionError::InvalidDigest`; `RetentionError::DigestMismatch`
...
- **Source/manifest evidence:** source tree SHA-256 `851773cef587a9c2f0d7dacf4e0d8497928cf5726a62eae2ca227e40fd0a2119`; manifest SHA-256 `a5cea0533a315670560cf62a20d7d465ae12f06c884b7c07ffcd0cfc77d8293f`.
