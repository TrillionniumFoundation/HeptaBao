# heptabao-proxy module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-proxy`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** local proxy credential and header boundary
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-proxy`; Rust files: `crates/heptabao-proxy/src/lib.rs`.
...
- **Public design surface:** const `MAX_HEADER_NAME_BYTES`; const `MAX_HEADER_VALUE_BYTES`; struct `HeaderName`; fn `parse`; fn `as_str`; struct `HeaderValue`; fn `new`; fn `expose`; struct `ProxyRequest`; struct `ForwardPlan`; struct `ProxyPolicy`; fn `new`; fn `plan_request`; fn `plan_response`; enum `ProxyError`
...
The source-defined failure vocabulary is: `ProxyError::InvalidPolicy`; `ProxyError::InvalidHeaderName`; `ProxyError::InvalidHeaderValue`; `ProxyError::DuplicateHeader`; `ProxyError::InvalidConnectionHeader`; `ProxyError::RequestTooLarge`; `ProxyError::ResponseTooLarge`
...
- **Source/manifest evidence:** portable repository-relative source SHA-256 `e806685b0d39568b76afaf2164ac53b1474e9c00bf1c6d054fb89bfa7b4e82dd`; manifest SHA-256 `da605afd3686bd1ab717dc5c8b8f52be923dcfc65d6838c5e57fdf70e55a7392`.
- **Named executable anchor:** `inbound_credentials_are_replaced_by_the_server_token` in `crates/heptabao-proxy/src/lib.rs`.
- **Required command:** `cargo +1.98.0 test --locked -p heptabao-proxy` (must be executed against this exact source tree; historical CI output is not current evidence).
