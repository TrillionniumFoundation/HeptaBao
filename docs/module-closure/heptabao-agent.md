# heptabao-agent module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-agent`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** auto-auth renewal and token sink lifecycle
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-agent`; Rust files: `crates/heptabao-agent/src/lib.rs`.
...
- **Public design surface:** enum `CredentialSource`; enum `TokenSinkKind`; struct `TokenSink`; fn `new`; const `fn`; fn `reference`; struct `AgentConfig`; enum `AgentState`; enum `ReconciledAgentState`; struct `AgentSession`; fn `new`; const `fn`; const `fn`; const `fn`; const `fn`; fn `requires_reconciliation`; fn `start`; fn `authentication_succeeded`; fn `authentication_failed_before_entry`; fn `retry_authentication`; fn `begin_renewal`; fn `renewal_succeeded`; fn `begin_revocation`; fn `revocation_succeeded`; fn `mark_outcome_unknown_after_entry`; fn `reconcile`; enum `AgentError`
...
The source-defined failure vocabulary is: `AgentError::InvalidConfig`; `AgentError::InvalidState`; `AgentError::InvalidTtl`; `AgentError::RetryNotDue`; `AgentError::SessionExpired`; `AgentError::MissingReconciliationReference`; `AgentError::GenerationRegression`; `AgentError::GenerationOverflow`; `AgentError::TickOverflow`
...
- **Source/manifest evidence:** portable repository-relative source SHA-256 `cca0b318de21fa48c368b0d3cdf0dab9216f3189ca6de900bf54af7c621f5138`; manifest SHA-256 `081f9b1eb2c4647064ae04f7f44d4328767af72cd133b491add40e8cb38fede5`.
- **Named executable anchor:** `authentication_renewal_and_revocation_are_monotonic` in `crates/heptabao-agent/src/lib.rs`.
- **Required command:** `cargo +1.98.0 test --locked -p heptabao-agent` (must be executed against this exact source tree; historical CI output is not current evidence).
