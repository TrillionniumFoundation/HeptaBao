# heptabao-barrier-api module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-barrier-api`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** barrier contracts
- **Repository state:** `INHERITED_IMPLEMENTED`.
- **Source root:** `crates/heptabao-barrier-api`; Rust files: `crates/heptabao-barrier-api/src/lib.rs`.
- **Internal dependencies:** `heptabao-storage-api`.
...
- **Public design surface:** const `SEALED_ENVELOPE_VERSION`; const `MAX_BARRIER_FIELD_BYTES`; const `MAX_ASSOCIATED_DATA_BYTES`; struct `KeyEpoch`; const `INITIAL`; const `fn`; const `fn`; const `fn`; enum `BarrierPurpose`; struct `BarrierContext`; fn `new`; fn `domain`; const `fn`; const `fn`; const `fn`; fn `canonical_associated_data`; struct `SecretState`; fn `new`; fn `as_bytes`; fn `len`; fn `is_empty`; fn `into_bytes`; struct `SealedEnvelope`; fn `new`; const `fn`; const `fn`; fn `nonce`; fn `ciphertext`; fn `authentication_tag`; fn `encode`
...
The source-defined failure vocabulary is: `BarrierContractError::ZeroKeyEpoch`; `BarrierContractError::KeyEpochOverflow`; `BarrierContractError::InvalidSecretState`; `BarrierContractError::AssociatedDataTooLarge`; `BarrierContractError::LengthOverflow`; `BarrierContractError::UnsupportedEnvelopeVersion`; `BarrierContractError::InvalidEnvelopeShape`; `BarrierContractError::TruncatedEnvelope`; `BarrierContractError::TrailingEnvelopeBytes`
...
- **Source/manifest evidence:** source tree SHA-256 `b22e9ce5124299cb9e8470e67391e5ca1d3ad94b814dec5c62575442a3a0431d`; manifest SHA-256 `94736e67ba9eb871a7d923adc01ad5ffc035909020c9ef8c115ffb43716457f9`.
