# heptabao-domain module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-domain`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** shared bounded domain values
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-domain`; Rust files: `crates/heptabao-domain/src/lib.rs`.
...
- **Public design surface:** const `MAX_ID_BYTES`; const `MAX_PATH_BYTES`; const `MAX_SECRET_BYTES`; enum `DomainError`; struct `Id`; fn `parse`; fn `as_str`; struct `CanonicalPath`; fn `parse`; fn `root`; fn `as_str`; fn `child`; fn `matches_prefix`; fn `relative_to`; struct `Tick`; const `fn`; const `fn`; fn `checked_add`; struct `SecretValue`; fn `new`; fn `expose`; fn `len`; fn `is_empty`
...
The source-defined failure vocabulary is: `DomainError::EmptyIdentifier`; `DomainError::IdentifierTooLong`; `DomainError::InvalidIdentifierCharacter`; `DomainError::InvalidIdentifierBoundary`; `DomainError::EmptyPath`; `DomainError::PathTooLong`; `DomainError::PathMustBeAbsolute`; `DomainError::InvalidPathSegment`; `DomainError::InvalidPathCharacter`; `DomainError::EmptySecret`; `DomainError::SecretTooLarge`; `DomainError::TickOverflow`
...
- **Source/manifest evidence:** source tree SHA-256 `f902ebcdaf70b3d07b0911f7409c6a6e1382da3ad089ded8738a11416432ee03`; manifest SHA-256 `afe7747976c6259d90b17132039ee7431ef80ec409b5a4e84fc1018e6576a8fd`.
