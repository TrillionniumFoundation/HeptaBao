# heptabao-runtime-service module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-runtime-service`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** authorized audit-to-durable mutation adapter
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-runtime-service`; Rust files: `crates/heptabao-runtime-service/src/lib.rs`.
- **Internal dependencies:** `heptabao-durable-service`.
...
- **Public design surface:** struct `Credential`; fn `new`; fn `expose`; enum `OperationKind`; enum `InboundOperation`; const `fn`; struct `InboundMutation`; fn `new`; struct `AuthenticatedPrincipal`; fn `new`; fn `as_str`; struct `AuthorizationDigest`; fn `new`; const `fn`; trait `Authenticator`; trait `Authorizer`; struct `AuthenticationFailure`; struct `AuthorizationFailure`; struct `AuditFailure`; enum `AuditStage`; struct `AuditEvent`; const `fn`; const `fn`; const `fn`; trait `AuditSink`; enum `RuntimeOutcome`; enum `RuntimeError`; struct `RuntimeService`; const `fn`; fn `handle`
...
The source-defined failure vocabulary is: `RuntimeOutcome::Committed`; `RuntimeOutcome::generation`; `RuntimeOutcome::recovery_reference`; `RuntimeError::InvalidRequest`; `RuntimeError::AuthenticationDenied`; `RuntimeError::AuthorizationDenied`; `RuntimeError::AuditUnavailableBeforeEntry`; `RuntimeError::AuditUnavailableAfterRead`; `RuntimeError::RecoveryRequired`; `RuntimeError::OutcomeUnknown`
...
- **Source/manifest evidence:** source tree SHA-256 `c348f4c1fa73bddd6554befdd2f0c8b45f0690fbb894149e211438071795e30d`; manifest SHA-256 `c8321f283c18f1bdb47e88910d8d0ac052f62eb247b785b6076e81ad6878f927`.
