# heptabao-token module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-token`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** token lifecycle
- **Repository state:** `IMPLEMENTED_REVIEW_REQUIRED`.
- **Source root:** `crates/heptabao-token`; Rust files: `crates/heptabao-token/src/lib.rs`.
...
- **Public design surface:** struct `TokenId`; fn `parse`; struct `TokenView`; struct `TokenStore`; fn `issue`; fn `validate`; fn `renew`; fn `revoke`; fn `revoke_entity`; enum `TokenError`
...
The source-defined failure vocabulary is: `TokenError::InvalidTtl`; `TokenError::DuplicateToken`; `TokenError::MissingToken`; `TokenError::Revoked`; `TokenError::Expired`; `TokenError::NotRenewable`
...
- **Source/manifest evidence:** source tree SHA-256 `63c5222f5b846667f7fb539461dc5cdaed9fdd83adfedd23d504769594d99e06`; manifest SHA-256 `1e3ce2af36291d20f00c54855df8c2f38baa662ad2f29efb1e2ab130c445ade5`.
