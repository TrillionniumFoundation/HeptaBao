# heptabao-single-node-journal module closure dossier

This dossier is the independently reviewable design, boundary, failure-semantics, and acceptance record for **`heptabao-single-node-journal`**. It is generated from the exact candidate tree and must be reviewed whenever the source or manifest hash changes. It does not grant compatibility, production, migration, or release authority.

## Design and state ownership

- **Capability domain:** local durable journal
- **Repository state:** `INHERITED_IMPLEMENTED`.
- **Source root:** `crates/heptabao-single-node-journal`; Rust files: `crates/heptabao-single-node-journal/src/lib.rs`.
- **Internal dependencies:** `heptabao-filesystem-guard`, `heptabao-journal-api`.
...
- **Public design surface:** const `MAX_JOURNAL_RECORDS`; struct `FileDurableJournal`; fn `create_new`; fn `reopen_existing`; fn `root`; fn `root_identity`; fn `reconcile_next_orphan`; enum `FileJournalError`
...
The source-defined failure vocabulary is: `DecodeError::Truncated`; `DecodeError::Trailing`; `DecodeError::InvalidMagic`; `DecodeError::InvalidLength`; `DecodeError::InvalidUtf8`; `DecodeError::InvalidSequence`; `DecodeError::InvalidTag`; `DecodeError::InvalidPreviousTag`; `DecodeError::InvalidPayload`; `TestAuthError::Contract`
...
- **Source/manifest evidence:** source tree SHA-256 `7c606e400dfa7dd0f6c4ce11a104b6650bbd65efc8d5cc776436daf208982971`; manifest SHA-256 `01ff4f6fd366388129bf4694d126102e209898a924f65f8fd957029c06168a6f`.
- **Named executable anchor:** `create_append_replay_and_reopen_round_trip` in `crates/heptabao-single-node-journal/src/lib.rs`.
- **Required command:** `cargo +1.98.0 test --locked -p heptabao-single-node-journal` (must be executed against this exact source tree; historical CI output is not current evidence).
