...
| Optional HTTPS audit collector | The same authenticated audit record, delivered only to a process-configured host-enrolled TLS endpoint after the local record is fsynced | One bounded no-retry delivery; redirect/unapproved origin/TLS failure blocks admission and fences audit until restart/recovery |
...
`audit_file` is the active segment. Configuration `audit.segment_bytes` (4096–33554432, default 33554432) and `audit.retained_segments` (1–64, default 8) bound automatic retention. The signed rotation manifest binds segment frontiers and an evicted-prefix anchor; it cannot reconstruct deleted events. Inspect files read-only and preserve the key/checkpoints. When `audit_http_url` is configured, the destination must already be present in the immutable pre-unseal outbound allowlist; runtime API requests may inspect but cannot redirect, replace or disable it. Collector delivery is intentionally synchronous and fail-closed after the local fsync. A failed remote delivery is retained in the local authenticated chain but is not blindly replayed on restart, so operators must treat the resulting remote sequence gap as an incident requiring local-chain reconciliation. Long-term archival, alert routing and retention policy remain deployment responsibilities, detailed in the [operator runbook](HEPTABAO_SINGLE_NODE_OPERATOR_RUNBOOK_V1.md).
