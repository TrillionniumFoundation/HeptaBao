...
pub trait ConsensusDriver: fmt::Debug {
    ...
    fn execute_leader_mutation(&mut self, operation: &ClientOperation) -> Result<ClientResult, HaError>;
    fn execute_linearizable_read(&mut self, operation: &ClientOperation) -> Result<ClientResult, HaError>;
}
    ...
    F: ForwardClient,
{
  ...
  match operation.kind {
      ...
      OperationKind::LinearizableRead => self.driver.execute_linearizable_read(operation)?,
  }
        } else {
  self.forwarder
      .forward(&view.leader, view.term, operation)?
        };
...
impl SnapshotManifest {
  ...
  || chunks.iter().any(|chunk| chunk.is_empty() || chunk.len() > MAX_PAYLOAD_BYTES)
        {
  return Err(HaError::InvalidSnapshot);
        }
        let total_bytes = chunks.iter().try_fold(0_u64, |total, chunk| {
  total.checked_add(chunk.len() as u64).ok_or(HaError::InvalidSnapshot)
        })?;
  ...
  value.checked_add(chunk.len() as u64).ok_or(HaError::InvalidSnapshot)
        })?;
...
impl TcpPeerTransport {
  ...
  return Err(HaError::InvalidFrame);
        ...
        let mut stream = TcpStream::connect_timeout(address, self.timeout).map_err(|_| HaError::Transport)?;
        stream.set_read_timeout(Some(self.timeout)).map_err(|_| HaError::Transport)?;
        stream.set_write_timeout(Some(self.timeout)).map_err(|_| HaError::Transport)?;
    ...
    H: FnOnce(Vec<u8>) -> Result<Vec<u8>, HaError>,
{
    ...
    stream.set_read_timeout(Some(timeout)).map_err(|_| HaError::Transport)?;
    stream.set_write_timeout(Some(timeout)).map_err(|_| HaError::Transport)?;
...
fn encode_envelope_body(
    ...
    let mut body = Vec::with_capacity(5 + 2 + cluster_id.len() + 2 + sender.as_str().len() + 2 + receiver.as_str().len() + 8 + 8 + 1 + 4 + payload.len());
...
fn encode_peer_state(
    ...
    for peer in peers {
        ...
        body.extend_from_slice(&incoming.get(&peer).copied().unwrap_or_default().to_be_bytes());
        body.extend_from_slice(&outgoing.get(&peer).copied().unwrap_or_default().to_be_bytes());
    }
    let tag = hmac::sign(&hmac::Key::new(hmac::HMAC_SHA256, &key), &body);
...
fn decode_peer_state(
    ...
    for _ in 0..count {
        let peer = NodeId::parse(take_string(&bytes[..body_end], &mut cursor, MAX_NODE_ID_BYTES)?)?;
...
fn create_private_root(root: &Path) -> Result<(), HaError> {
  ...
  builder.create(root).map_err(|_| HaError::Io)?;
    ...
    #[cfg(unix)]
    {
        ...
        if fs::metadata(root).map_err(|_| HaError::Io)?.permissions().mode() & 0o077 != 0 {
  return Err(HaError::InvalidRoot);
        }
    }
    Ok(())
...
fn read_frame(stream: &mut TcpStream) -> Result<Vec<u8>, HaError> {
    ...
    stream.read_exact(&mut length).map_err(|_| HaError::Transport)?;
    let length = u32::from_be_bytes(length) as usize;
    if length == 0 || length > MAX_PAYLOAD_BYTES + 4096 {
        return Err(HaError::InvalidFrame);
    }
    let mut frame = vec![0_u8; length];
    stream.read_exact(&mut frame).map_err(|_| HaError::Transport)?;
    Ok(frame)
}
...
mod tests {
    ...
    impl ConsensusDriver for MockDriver {
        ...
        fn leader_view(&self) -> Result<LeaderView, HaError> { Ok(self.view.clone()) }
        fn execute_leader_mutation(&mut self, operation: &ClientOperation) -> Result<ClientResult, HaError> {
  self.writes += 1;
  ClientResult::new(self.view.term, self.view.commit_index + 1, operation.payload().to_vec())
        }
        fn execute_linearizable_read(&mut self, operation: &ClientOperation) -> Result<ClientResult, HaError> {
  self.reads += 1;
  ClientResult::new(self.view.term, self.view.commit_index, operation.payload().to_vec())
        }
    }

    #[derive(Debug, Default)]
    struct MockForwarder { calls: usize }

    impl ForwardClient for MockForwarder {
        fn forward(&mut self, _leader: &NodeId, expected_term: u64, operation: &ClientOperation) -> Result<ClientResult, HaError> {
  self.calls += 1;
  ClientResult::new(expected_term, 10, operation.payload().to_vec())
        }
    }

    fn node(value: &str) -> NodeId { NodeId::parse(value).unwrap() }

    fn root(name: &str) -> PathBuf {
        let nonce = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos();
        let root = std::env::temp_dir().join(format!("heptabao-ha-service-{name}-{}-{nonce}", std::process::id()));
        #[cfg(unix)]
        {
  use std::os::unix::fs::DirBuilderExt;
  let mut builder = fs::DirBuilder::new();
  ...
  builder.create(&root).unwrap();
    ...
    fn leader_executes_and_follower_forwards_with_deduplication() {
        let leader = node("n1");
        let driver = MockDriver { local: leader.clone(), view: LeaderView { leader: leader.clone(), term: 3, commit_index: 9, quorum_available: true }, writes: 0, reads: 0 };
        let mut service = HaService::new(driver, MockForwarder::default(), 32).unwrap();
        let operation = ClientOperation::new(OperationId([1;16]), OperationKind::Mutation, b"write".to_vec()).unwrap();
        ...
        let follower = MockDriver { local: node("n2"), view: LeaderView { leader, term: 4, commit_index: 10, quorum_available: true }, writes: 0, reads: 0 };
        let mut service = HaService::new(follower, MockForwarder::default(), 32).unwrap();
        let operation = ClientOperation::new(OperationId([2;16]), OperationKind::LinearizableRead, b"read".to_vec()).unwrap();
    ...
    fn quorum_loss_and_operation_id_conflict_fail_closed() {
        let local = node("n1");
        let driver = MockDriver { local: local.clone(), view: LeaderView { leader: local, term: 1, commit_index: 1, quorum_available: false }, writes: 0, reads: 0 };
        let mut service = HaService::new(driver, MockForwarder::default(), 4).unwrap();
        let operation = ClientOperation::new(OperationId([3;16]), OperationKind::Mutation, b"x".to_vec()).unwrap();
        assert_eq!(service.execute(&operation), Err(HaError::QuorumUnavailable));
    }

    #[test]
    fn authenticated_peer_replay_is_rejected_after_restart() {
        let n1=node("n1"); let n2=node("n2");
        let auth=PeerAuthenticator::new("cluster-a", BTreeMap::from([(n1.clone(),[1;32]),(n2.clone(),[2;32])])).unwrap();
        let envelope=auth.seal(n1.clone(),n2.clone(),7,1,PeerMessageKind::ForwardRequest,b"request".to_vec()).unwrap();
  ...
  assert_eq!(admit_peer_envelope(&n2,&auth,&mut sequences,&envelope),Err(HaError::ReplayDetected));
        }
        let mut reopened=PersistentPeerSequences::open(&root,[3;32]).unwrap();
        assert_eq!(admit_peer_envelope(&n2,&auth,&mut reopened,&envelope),Err(HaError::ReplayDetected));
        drop(reopened); drop(auth); fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn peer_frame_tampering_and_wrong_receiver_are_rejected() {
        let n1=node("n1"); let n2=node("n2"); let n3=node("n3");
        let auth=PeerAuthenticator::new("cluster-a",BTreeMap::from([(n1.clone(),[4;32]),(n2.clone(),[5;32]),(n3.clone(),[6;32])])).unwrap();
        let envelope=auth.seal(n1,n2.clone(),2,9,PeerMessageKind::ReadIndex,b"x".to_vec()).unwrap();
        assert_eq!(auth.verify(&n3,&envelope),Err(HaError::PeerAuthenticationFailed));
        let mut encoded=encode_peer_envelope(&envelope).unwrap();
        let index=encoded.len()/2; encoded[index]^=0x10;
        let tampered=decode_peer_envelope(&encoded).unwrap();
        assert_eq!(auth.verify(&n2,&tampered),Err(HaError::PeerAuthenticationFailed));
    }
    ...
    fn snapshot_and_joint_membership_are_verified() {
        ...
        let mut changed=chunks.clone(); changed[1][0]^=1;
    ...
    fn tcp_transport_uses_bounded_framing() {
        ...
        let server=thread::spawn(move || serve_one_peer_frame(&listener,Duration::from_secs(2),Ok).unwrap());
        let transport=TcpPeerTransport::new(BTreeMap::from([(node("n2"),address)]),Duration::from_secs(2)).unwrap();
