...
impl AuthState {
    ...
    pub(super) fn jwt_token_expiry(
        ...
        if role.token_period == 0 {
            // Ordinary role/mount maxima are read anew and measured from
            // issue time. Raising them can extend a still-active token beyond
            // its old role maximum; only explicit_max is captured at issue.
