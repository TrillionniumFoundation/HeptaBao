...
pub(crate) struct EffectPlan {
    ...
    pub original_attributes: Vec<(String, Vec<String>)>,
...
fn validate_template(source: &str, expect_delete: bool) -> std::result::Result<(), EngineError> {
    if source.is_empty() || source.len() > 32 * 1024 || source.contains('\r') {
        return Err(err(
            400,
            "LDAP LDIF template is outside the bounded profile",
        ));
    }
    let mut dn = None;
    let mut change = None;
    ...
    for line in source.lines() {
        if line.is_empty() {
            return Err(err(
                400,
                "LDAP dynamic profile supports exactly one LDIF entry",
            ));
        }
        if line.starts_with(' ') || line.starts_with('\t') || line.contains("::") {
            return Err(err(
                400,
                "LDAP folded/base64 LDIF is outside the bounded profile",
            ));
        }
    ...
    if expect_delete {
        if change != "delete" || attributes != 0 {
            return Err(err(
                400,
                "LDAP deletion/rollback LDIF must be one delete entry",
            ));
        }
    } else if change != "add" || attributes == 0 || !source.contains("{{.Password}}") {
        return Err(err(
            400,
            "LDAP creation LDIF must add an entry and bind generated password",
        ));
    }
    let stripped = source
        .replace("{{.Username}}", "")
...
impl OpenLdap {
    pub(crate) fn contains_lease(&self, lease_id: &str) -> bool {
    ...
    pub(crate) fn reconcile_candidates(
        ...
        self.leases
            .iter()
            .filter_map(|(id, lease)| {
                let force_revoke = lease.expires_at <= now || !live_owners.contains(&lease.owner);
                (lease.phase != Phase::Active || force_revoke).then_some((id.clone(), force_revoke))
            })
            .collect()
    }
    ...
    pub(crate) fn prepare_effect(
        ...
        if self
            .leases
            .get(lease_id)
            .is_some_and(|lease| lease.expires_at <= now && lease.phase != Phase::PendingRevoke)
        {
            self.leases
                .get_mut(lease_id)
                .ok_or_else(|| err(404, "OpenLDAP lease not found"))?
        ...
        let lease = self
            ...
            .ok_or_else(|| err(404, "OpenLDAP lease not found"))?
            .clone();
        let action = match lease.phase {
            Phase::PendingIssue => EffectAction::Issue,
            Phase::PendingRevoke => EffectAction::Revoke,
            Phase::Active => {
                return Err(err(409, "OpenLDAP lease does not need reconciliation"));
            }
        };
        let (entry, original_attributes) = if action == EffectAction::Issue {
            let role = self
                .roles
                .get(&lease.role)
                .ok_or_else(|| err(503, "OpenLDAP role disappeared after durable intent"))?;
            let mut entry = render_template(
                &role.creation_ldif,
                &lease.username,
                lease.password.expose(),
                &config.userdn,
                false,
            )?;
            if entry.dn != lease.dn
                || entry
                    .attributes
                    .iter()
                    .any(|(name, _)| name.eq_ignore_ascii_case("description"))
            {
                return Err(err(503, "OpenLDAP issue intent no longer matches its role"));
            }
            entry.attributes.push((
                "description".to_owned(),
                vec![format!("hb-request:{}", lease.request_digest)],
            ));
            let original_attributes = entry
                .attributes
                .iter()
                .filter(|(name, _)| !name.eq_ignore_ascii_case("description"))
                .cloned()
                .collect();
            (Some(entry), original_attributes)
        } else {
            let role = self
                .roles
                .get(&lease.role)
                .ok_or_else(|| err(503, "OpenLDAP role disappeared before revoke"))?;
    ...
    pub(crate) fn dispatch(
            ...
            return match method {
                ...
                "POST" | "PUT" => {
                    if object.keys().any(|key| {
                        !matches!(
                            key.as_str(),
                            "url" | "binddn" | "bindpass" | "userdn" | "schema"
                        )
                    }) {
        ...
        if let Some(role_name) = relative.strip_prefix("creds/") {
            ...
            if method != "GET" || !object.is_empty() {
                return Err(err(
                    405,
                    "OpenLDAP dynamic credentials require an empty GET request",
                ));
            }
            let issuer =
                issuer.ok_or_else(|| err(403, "OpenLDAP credentials require a live issuer"))?;
            let config = self
                .config
                .as_ref()
                    ...
                    >= MAX_PENDING
            {
                return Err(err(
                    507,
                    "OpenLDAP lease or pending-effect capacity exhausted",
                ));
            }
            let entropy = hex(&crypto::random::<8>()
            ...
            let password = hex(&crypto::random::<24>()
                .map_err(|_| err(503, "operating system randomness unavailable"))?);
            let mut entry = render_template(
                &role.creation_ldif,
                &username,
            ...
            if deletion.dn != entry.dn {
                return Err(err(
                    400,
                    "OpenLDAP creation/deletion LDIF target different DNs",
                ));
            }
            ...
            let ttl = issuer.expires_at.map_or(role.default_ttl, |limit| {
                limit.saturating_sub(now).min(role.default_ttl)
            });
            if ttl < MIN_TTL {
                return Err(err(
                    403,
                    "OpenLDAP issuer lifetime is too short for a new lease",
                ));
            }
            let expires_at = now
                .checked_add(ttl)
                ...
                .any(|(name, _)| name.eq_ignore_ascii_case("description"))
            {
                return Err(err(
                    400,
                    "OpenLDAP creation LDIF must leave description available for the durable fence",
                ));
            }
            entry.attributes.push((
                "description".to_owned(),
                vec![format!("hb-request:{request}")],
            ));
    ...
    pub(crate) fn finalize(
            ...
            || lease.username != plan.username
        {
            return Err(err(
                503,
                "OpenLDAP durable intent changed after provider entry",
            ));
        }
