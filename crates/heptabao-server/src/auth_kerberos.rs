...
impl KerberosMount {
    ...
    fn config_revision(&self) -> String {
        let policies = self.policies.iter().cloned().collect::<Vec<_>>();
        hash(&format!(
            "heptabao-kerberos-v1\0provider={KERBEROS_PROVIDER_IDENTITY}\0service_account={}\0realm={}\0service={}\0keytab_path={}\0policies={:?}\0token_ttl={}\0token_max_ttl={}\0token_explicit_max_ttl={}\0token_num_uses={}\0clock_skew_seconds={}",
            self.service_account,
            self.realm,
            self.service,
            self.keytab_path,
...
impl AuthState {
    ...
    pub(crate) fn prepare_kerberos_login(
        ...
        let encoded = authorization
            ...
            .ok_or_else(|| bad("invalid Kerberos authorization"))?;
        if encoded.len() > MAX_KERBEROS_TOKEN * 2 || STANDARD.decode(encoded).is_err() {
            return Err(bad("invalid Kerberos authorization"));
        }
    ...
    pub(crate) fn finish_kerberos_login(
        ...
        let (token_id, mut token, mut response) = Self::prepare_issue(token, now)?;
        config
            .replay
        ...
        let provider = KERBEROS_PROVIDER_IDENTITY.to_owned();
        let service_account = config.service_account.clone();
        let realm = config.realm.clone();
        let service = config.service.clone();
        let mount_revision = plan.mount_revision.revision;
        let metadata = BTreeMap::from([
            ("principal".into(), observation.principal.clone()),
            ("realm".into(), observation.realm.clone()),
            ("service".into(), observation.service.clone()),
            ("service_account".into(), service_account.clone()),
            ("provider".into(), provider.clone()),
            ("mount_revision".into(), mount_revision.to_string()),
            ("config_revision".into(), config_revision.clone()),
        ]);
        token.auth_provenance = Some(TokenAuthProvenance::Kerberos {
            provider,
            service_account,
            realm: realm.clone(),
            service,
        ...
        response.body["auth"]["metadata"] = json!({
            "principal": observation.principal,
            "realm": realm,
            "service": observation.service,
            "provider": KERBEROS_PROVIDER_IDENTITY,
