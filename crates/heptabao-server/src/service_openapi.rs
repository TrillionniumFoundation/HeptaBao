...
const FIXED_ROUTES: &[Route] = &[
    ...
    Route {
        ...
        methods: &["get"],
        description: "List bounded, namespace-owned workflow profiles.",
        sudo: false,
        unauthenticated: false,
    },
    Route {
        path: "/sys/workflows/profiles/{name}",
        methods: &["get", "post", "put", "delete"],
        description: "Read or mutate a bounded workflow profile using only registered internal operations.",
        sudo: false,
        unauthenticated: false,
    },
    Route {
        path: "/sys/workflows/profiles/{name}/runs",
        methods: &["get", "post"],
        description: "Inspect or execute a durable, idempotent workflow run.",
        sudo: false,
        unauthenticated: false,
    },
    Route {
        path: "/sys/workflows/runs/{id}",
        methods: &["get"],
        description: "Read a namespace-owned workflow run summary without secret output.",
        sudo: false,
        unauthenticated: false,
    },
    Route {
        path: "/sys/workflows/runs/{id}/reconcile",
        methods: &["get", "post"],
        description: "Inspect post-entry workflow uncertainty; automatic replay is disabled.",
        sudo: false,
        unauthenticated: false,
    },
    Route {
