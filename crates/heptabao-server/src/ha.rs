...
impl HaProcess {
    pub fn start(config: HaProcessConfig) -> Result<Self, String> {
        ...
        let peer_ids = parsed_peers.iter().map(|peer| peer.id).collect::<BTreeSet<_>>();
        ...
        let node = if existing {
            runtime
                .block_on(ProcessRaftNode::reopen(&config.raft_dir, config.node_id, network))
                .map_err(|error| error.to_string())?
        } else {
            runtime
                .block_on(ProcessRaftNode::create(&config.raft_dir, config.node_id, network))
                .map_err(|error| error.to_string())?
        };
            ...
            .spawn(move || {
                while !listener_stop.load(Ordering::Acquire) {
                    ...
                    let result = serve_one_mtls_peer_frame(
                        ...
                        timeout,
                        |peer, frame| {
                            let source = *ids.get(&peer).ok_or(heptabao_ha_service::HaError::UnknownPeer)?;
...
fn build_mutual_tls(
    ...
    let key_bytes = read_bounded_regular_file(&config.key_file, MAX_TLS_FILE_BYTES)?;
...
fn read_bounded_regular_file(path: &Path, maximum: usize) -> Result<Vec<u8>, String> {
    ...
    let file = options.open(path).map_err(|_| "cannot open HA TLS file".to_owned())?;
    let metadata = file
        .metadata()
        .map_err(|_| "cannot inspect HA TLS file".to_owned())?;
    if !metadata.is_file() || metadata.len() > maximum as u64 {
        return Err("HA TLS material must be a bounded regular file".into());
    }
    let mut bytes = Vec::new();
    file.take(u64::try_from(maximum).map_err(|_| "HA TLS file bound overflow".to_owned())? + 1)
        .read_to_end(&mut bytes)
        .map_err(|_| "cannot read HA TLS file".to_owned())?;
    if bytes.len() > maximum {
        return Err("HA TLS material exceeds limit".into());
    }
    Ok(bytes)
}
...
fn encode_raft_frame(frame: RaftWireFrame) -> Result<Vec<u8>, RemoteRaftError> {
    ...
    let payload_length = u32::try_from(frame.payload.len()).map_err(|_| RemoteRaftError::InvalidRpc)?;
...
fn decode_raft_frame(encoded: &[u8]) -> Result<RaftWireFrame, RemoteRaftError> {
    if encoded.len() < 27 || encoded.len() > MAX_RAFT_FRAME_BYTES || &encoded[..5] != RAFT_FRAME_MAGIC {
        return Err(RemoteRaftError::InvalidRpc);
    }
    let role = encoded[5];
    ...
    let source = u64::from_be_bytes(encoded[6..14].try_into().map_err(|_| RemoteRaftError::InvalidRpc)?);
    let target = u64::from_be_bytes(encoded[14..22].try_into().map_err(|_| RemoteRaftError::InvalidRpc)?);
    let kind = decode_kind(encoded[22])?;
    let length = u32::from_be_bytes(encoded[23..27].try_into().map_err(|_| RemoteRaftError::InvalidRpc)?) as usize;
    if source == 0
        || target == 0
        || source == target
        || length == 0
