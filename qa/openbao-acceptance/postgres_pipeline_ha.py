...
class ProviderCluster(Cluster):
    ...
    def run(self):
        ...
        self.check('db_ha.provider_model_disabled',next(r for r in p.rows.values() if r['username']==user)['login'] is False)
        self.restart(leader);leader=self.leader()
        self.check('db_ha.old_leader_cannot_resurrect',call(leader,'POST','sys/leases/lookup',dict(lease_id=identity))[1]['data']['phase']=='Revoked')
        ...
        while p.rows[provider_id]['login'] and time.monotonic()<deadline:time.sleep(.1)
        self.check('db_ha.new_leader_reconciles_without_client_request',p.rows[provider_id]['login'] is False)
        self.check('db_ha.pending_is_durable_terminal_after_reconcile',call(successor,'POST','sys/leases/lookup',dict(lease_id=identity))[1]['data']['phase']=='Revoked')
