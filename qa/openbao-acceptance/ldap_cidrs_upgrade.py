#!/usr/bin/env python3
"""Pinned schema28 to29 LDAP CIDR migration, real TLS and OpenLDAP Bind/Search."""
...
LEGACY_HARNESS='649c125af5c59064edbac4bfa5f10bc608b9e37d'
...
def admit_legacy_receipt(expected,receipt):
    if LEGACY_HARNESS is None or LEGACY_RECEIPT is None:raise ValueError('legacy_schema28_receipt_not_yet_pinned')
    identity=receipt.get('source_identity',{})
    if (expected!=LEGACY_SHA256 or receipt.get('status')!='passed' or receipt.get('source_and_binary_unchanged') is not True or receipt.get('cases_match') is not True or receipt.get('build_source_commit')!=LEGACY_SOURCE or identity.get('source_commit')!=LEGACY_HARNESS or identity.get('binary_sha256')!=LEGACY_SHA256 or identity.get('source_dirty') is not False):raise ValueError('legacy_schema28_build_receipt_mismatch')
...
def prepare_legacy(instance,provider,cases):
    ...
    if status!=200:raise ScenarioFailure('ldap_cidrs.upgrade.legacy.initialize')
    instance.token,key=initialized['root_token'],initialized['keys_base64'][0]
    t=Trace(SourceClient(instance.address,instance.root/'ca.crt',instance.token),provider,configuration('candidate',provider,(instance.root/'ca.crt').read_text()),cases)
    ...
    t.config('upgrade.legacy.config',dict(t.configuration,token_ttl=300,token_max_ttl=3600,token_policies=['cidr-issuer']))
...
def complete_scenarios(rows):
    ...
    return bool(rows) and all(r.get('passed') is True for r in rows) and len(names)==len(set(names)) and {'ldap_cidrs.'+n for n in MILESTONES}.issubset(names) and names[-1]=='ldap_cidrs.upgrade.complete'
...
def main():
    ...
    if LEGACY_RECEIPT is None:raise ValueError("legacy_schema28_receipt_not_yet_pinned")
    ...
    if args.legacy_prepare_only:
        ...
        if candidate_hash != LEGACY_SHA256:
            raise ValueError("legacy_schema28_binary_mismatch")
    ...
    root = Path(tempfile.mkdtemp(prefix="heptabao-ldap-cidrs-upgrade-"))
    root.chmod(0o700)
    instance = provider = None
    result = {"schema": "heptabao.ldap-cidrs-upgrade.v1", "from_schema": 28, "minimum_to_schema": 29,
