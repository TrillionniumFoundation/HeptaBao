...
from jwt_native_upgrade import LEGACY_SHA256, LEGACY_SOURCE, Trace, admit_legacy


class JwtNativeUpgradeTests(unittest.TestCase):
    def receipt(self):
    ...
    def test_old_binary_requires_the_observed_clean_build(self):
        ...
        with patch("jwt_native_upgrade.validate_binary_pins", return_value=("current", "legacy")) as pins:
    ...
    def test_failed_unseal_cannot_continue_as_success_or_print_credentials(self):
        ...
        cases = []
        with self.assertRaisesRegex(ScenarioFailure, "^jwt_native_upgrade.downgrade$"):
            Trace(Client(), cases).call("downgrade", "sys/unseal", {"key": "private-key"}, expected=503)
        self.assertEqual(cases, [{"case": "jwt_native_upgrade.downgrade", "status": 200, "passed": False}])
