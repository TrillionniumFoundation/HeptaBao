...
class ForwardingCompletionTests(unittest.TestCase):
    ...
    def test_ldap_requires_its_own_real_provider_and_election_evidence(self):
        names=sorted(fixture.MILESTONES-{'ha.complete'})+['ha.complete']
        rows=[{'case':'ldap_cidrs.'+n,'passed':True} for n in names]
        self.assertTrue(fixture.complete(rows,ldap=True))
        self.assertFalse(fixture.complete(rows))
        for required in ['ha.forwarded_wrong_login','ha.after_election_renew.self.shape','ha.health_only_catchup']:
            self.assertFalse(fixture.complete([r for r in rows if r['case']!='ldap_cidrs.'+required],ldap=True))
