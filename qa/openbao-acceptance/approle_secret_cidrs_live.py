#!/usr/bin/env python3
"""Compare role-level AppRole SecretID source CIDR behavior with the observed OpenBao2.6.2 contract."""
...
import approle_secret_cidrs_probe as contract

CALIBRATION_PATH = Path(__file__).parent/'evidence/approle-secret-cidrs-official-2df0658.json'
CALIBRATION_SHA256 = '49b4404b3e0231106f2cb00942d18d46fe8c4a75a66ddc8b3e09c84a4f579eed'
CONTRACT_SHA256 = 'ca9c2ab8963a73e1c01020b4904dd31781b4f81f5eae089ee5d8cef450e4ae0f'


def safe_rows(rows):
    if not isinstance(rows, list) or not rows: return False
    names = set()
    booleans = {'auth', 'data', 'wrap', 'errors', 'renewable', 'accessor', 'orphan',
                'bind_secret_id', 'source_ipv4', 'warnings'}
    scalar = {'case', 'status', 'secret_id_num_uses', 'num_uses', 'token_type'}
    prefixes = ('', 'role_login_', 'legacy_alias_', 'sid_login_')
    projections = {prefix+key for prefix in prefixes for key in ('cidr_shape', 'cidrs')}
    for row in rows:
        if not isinstance(row, dict): return False
        name = row.get('case')
        if not isinstance(name, str) or not re.fullmatch(r'[a-z0-9_.]{1,140}', name) or name in names: return False
        names.add(name)
        if row.get('not_run') is True:
            # No issued credential is itself an observed branch, not a passing
            # bearer check. Keep its position and exact identity in comparison.
            if set(row) != {'case', 'not_run', 'reason'} or row['reason'] != 'no_issued_token': return False
            if name not in {f'consume.{kind}.one.issued_foreign_use_pending' for kind in ('service', 'batch')}: return False
            continue
        if not {'case', 'status', 'auth', 'data', 'wrap', 'errors', 'source_ipv4', 'warnings'} <= row.keys(): return False
        ...
        if row['source_ipv4'] is not True: return False
        ...
        for prefix in prefixes:
            shape, values = prefix+'cidr_shape', prefix+'cidrs'
            if shape in row and row[shape] not in ('missing', 'null', 'list'): return False
            if values in row:
                if row.get(shape) != 'list' or not isinstance(row[values], list): return False
                try: contract.cidr_projection({contract.FIELD: row[values]}, contract.FIELD)
                except ValueError: return False
            elif row.get(shape) == 'list': return False
    return True
...
def calibrated_rows():
    ...
    if (value.get('schema') != 'heptabao.approle-secret-cidrs-oracle-probe.v1'
        ...
        or value.get('pending_cases') != [row['case'] for row in value['cases'] if row.get('not_run')]):
...
def main():
    ...
    work = Path(tempfile.mkdtemp(prefix='approle-secret-cidrs-dual-', dir=private_parent(args.work_parent)))
    ...
    report = {'schema': 'heptabao.approle-secret-cidrs-comparison.v1', 'status': 'passed' if passed else 'failed',
        'cases': cases, 'completed_scenarios': finished,
        'not_run_cases': {name: [row['case'] for row in rows if row.get('not_run')] for name, rows in cases.items()},
        'executed_case_counts': {name: sum(not row.get('not_run', False) for row in rows) for name, rows in cases.items()},
        'not_run_are_passed_assertions': False, 'processes_stopped': stopped, 'calibrated_cases_match': matches,
        ...
        'numeric_CIDR_profile_only': True, 'role_SecretID_login_CIDRs_covered': passed, 'SecretID_CIDR_overrides_covered': False,
