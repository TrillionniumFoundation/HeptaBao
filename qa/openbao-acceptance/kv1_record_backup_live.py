...
POSTGRES_REQUIRED = frozenset({'fresh_postgresql_owner', 'postgres_no_local_artifact_fallback',
    'postgres_single_authoritative_manifest', 'postgres_plaintext_absent'})
...
def complete(checks, *, postgres=False):
    ...
    required = REQUIRED | POSTGRES_REQUIRED if postgres else REQUIRED
    return len(names) == len(set(names)) and names[-1] == 'complete' and required.issubset(names)


def run(binary, work, checks, observations, postgres_bin=None):
    instance = pg = None
    ...
    try:
        ...
        if postgres_bin is not None:
            ...
            config['postgres_durable'] = dict(endpoint=dict(origin=pg.origin,
                ...
                username='hb_storage', password=pg.manager_password, scope='record-backup-fixture')
        private_write(config_path, config, replace=True)
        instance.start()
        params = {'secret_shares':1,'secret_threshold':1}
        if pg is not None: params['recovery_nonce'] = secrets.token_hex(32)
        ...
        if pg is not None:
            check('postgres_no_local_artifact_fallback',
            ...
            rows = pg.sql("SELECT count(*) FROM heptabao_durable_v1.manifest_v1 "
                          "WHERE scope='record-backup-fixture'")
            check('postgres_single_authoritative_manifest',
                  rows.returncode == 0 and rows.stdout.strip() == '1')
        instance.stop()
        ...
        if pg is not None:
            rows = pg.sql("SELECT encode(bytes,'hex') FROM heptabao_durable_v1.chunks_v1 "
                          "WHERE scope='record-backup-fixture'")
            check('postgres_plaintext_absent', rows.returncode == 0 and bool(rows.stdout.strip())
                  and all(not any(sample in bytes.fromhex(row) for sample in samples)
...
def main():
    ...
    try: run(binary,work,checks,observations,args.postgres_bin)
    ...
    if not complete(checks,postgres=args.postgres_bin is not None):failure=failure or 'incomplete_observations'
    report={'schema':'heptabao.kv1-record-backup.v1','status':'passed' if failure is None else 'failed',
        ...
        'durable_backend':'postgresql_17' if args.postgres_bin is not None else 'files',
