...
INPUTS = (
 ('type_number','{"a=x":"good","b=y":42}'),
 ('type_nested','{"a":"x=y","b":{"c":"d=e"}}'),
 ('type_array','{"a=x":"good","b=y":[]}'),
 ('type_number_first','{"b=y":42,"a=x":"good"}'),
 ('malformed_json','{"a=x":"good","b=y":'),
 ('json_null_value','{"a=x":"good","b=y":null}'),
 ('json_strings_control','{"a=x":"good","b=y":"fine"}'),
 ('csv_control','A=GOOD,B=FINE'),
 ('csv_missing_equals','a=good,z'),
)
...
def main():
    ...
    work = Path(tempfile.mkdtemp(prefix='approle-metadata-partial-', dir=private_parent(args.work_parent)))
    ...
    report = {'schema': 'heptabao.approle-metadata-partial-probe.v1', 'status': status,
